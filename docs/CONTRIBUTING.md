...
**Name a pack for the skill, not the occasion.** The `id`, filename, `name`, `documentTitle`, `share.title` and the hero eyebrow must describe the *skill or fault the pack trains* in generic terms - `non-standard-starts`, `reloads`, `stage-planning` - never a specific match, person, or event. A match-prep taper is still allowed; the match just belongs in the body copy as the worked example (diagnosis intro, drills, evidence, `references`, and `CREDITS.md`), not in the title. Keep `id` and filename identical - the build derives the `?pack=` slug and share-stub path from the filename stem, and the switcher matches it against the `id:` field at runtime.
