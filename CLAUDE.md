...
- **No `localStorage` / `sessionStorage` / IndexedDB** in the committed source. The deployed file is previewed in a sandbox that forbids browser storage. Persistence is a roadmap item and, when added, must degrade gracefully when storage is unavailable. Preferences that need to survive a reload (pack selection, audio profile) persist via query params + `history.replaceState` - `?pack=<id>`, `?sound=quiet` - never via storage. Two sanctioned exceptions use feature-detected `localStorage`, both degrading to session-only state when storage is unavailable so the committed file still runs in the no-storage preview: adaptive-par progression (`torreld.progress.v1`, see `docs/superpowers/specs/2026-06-30-adaptive-par-design.md`) and the landing picker's last-pack memory (`torreld.lastPack.v1`, see `docs/superpowers/specs/2026-07-02-landing-pack-picker-design.md`). Both feature-detect through the single `TORRELD.storage()` probe; when it returns `null`, storage is simply skipped. Active pack selection still rides the URL (`?pack=<id>`); `lastPack` only decides the initial view on a bare cold visit.
